...
class Graph:
    ...
    def fit_missing_links(self, X, method='xgboost'):
        ...
        for node in pbar:
            ...
            y_train = X[node.name].loc[pd.concat(train_mask)]
            X_test = X[[p.name for p in node.args]].loc[pd.concat(test_mask)]
            ...
            if method == 'xgboost':
                ...
                xgb_train_train = xgboost.DMatrix(X_train_train, label=y_train_train)
                xgb_train_val = xgboost.DMatrix(X_train_val, label=y_train_val)
                ...
                r2_new = r2_score(y_test, m_new.predict(xgb_test))
                r2_scores[node.name] = r2_new
